#####################################################
#   Changes in v3:                                  #
#       - add and/or functionality for keywords     #
#####################################################
          ...
          sidebarPanel(width=2,
               ...
               radioButtons(inputId = "analysisType", 
                            ...
                            choices = c("None selected",
                                        "Keyword vs no keyword analysis",
...
server <- function(input, output) {
    ...
    analysis_type <- reactive({
        if (input$analysisType == "None selected") {
            return(NULL)
        }
        if_else(input$analysisType == "Keyword vs no keyword analysis", 0, 1 )
    })
    ...
    output$dateRange2.ui <- renderUI({
        ...
        dateRangeInput(inputId = "dateRange2",
                       ...
                       start = as.character(as.Date(daterange2()[1])+1),
                       end = daterange2()[2])
    })
